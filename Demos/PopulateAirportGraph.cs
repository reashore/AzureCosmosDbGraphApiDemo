...
namespace CosmosDb.GremlinApi.Demos
{
	public static class PopulateAirportGraph
	{
		...
		private static async Task CreateTerminal(IGremlinClient client, string id)
		{
			string gremlinCode = $@"
				g.addV('terminal')
					.property('id', '{id}')
		...
		private static async Task CreateTerminalToGate(IGremlinClient client, string terminal, string gate, int distanceInMinutes)
		{
			string gremlinCode = $@"
				g.V()
					.has('id', '{terminal}')
		...
		private static async Task CreateTerminalToRestaurant(IGremlinClient client, string terminal, string restaurant, int distanceInMinutes)
		{
			string gremlinCode = $@"
				g.V()
					.has('id', '{terminal}')
